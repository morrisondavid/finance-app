...
describe('computeFinancialSafety', () => {
  ...
  it('earned receivables lift pillar A and resources-after-commitments', () => {
    const without = computeFinancialSafety(
      baseInput({ totalCommittedGbp: 100_000, cashAfterCommitmentsGbp: 0, earnedButNotCollectedGbp: 0 }),
    );
    const withEarned = computeFinancialSafety(
      baseInput({ totalCommittedGbp: 100_000, cashAfterCommitmentsGbp: 0, earnedButNotCollectedGbp: 25_000 }),
    );
  ...
  it('earnedButNotCollectedGbp of 0 leaves pillar A on cash-only behaviour', () => {
    const r = computeFinancialSafety(
      baseInput({ totalCommittedGbp: 100_000, cashAfterCommitmentsGbp: 50_000, earnedButNotCollectedGbp: 0 }),
    );
    expect(r.pillars[0].contribution).toBeGreaterThanOrEqual(8);
  });
