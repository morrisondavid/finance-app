...
describe('Integration Tests with Real Bank CSV Samples', () => {
  ...
  it('partitions real Barclays CSV with transaction integrity', () => {
    const samplePath = path.join(__dirname, '../../parsers/__tests__/fixtures/barclays-sample.csv');
  ...
  it('partitions real Barclaycard CSV with transaction integrity', () => {
    const samplePath = path.join(__dirname, '../../parsers/__tests__/fixtures/barclaycard-sample.csv');
    const csvContent = fs.readFileSync(samplePath, 'utf-8');
  ...
  it('handles all four bank formats simultaneously', () => {
    ...
    const results = samples.map(sample => {
      const samplePath = path.join(__dirname, '../../parsers/__tests__/fixtures', sample.path);
      const csvContent = fs.readFileSync(samplePath, 'utf-8');
...
05/02/2025,200.00,Filler`;
    ...
    const result = partitionCSVFile('/file.csv', 'test', mockFs);
