  ...
  const server = new McpServer(
    { name: 'bank-statements-ai', version: '2.0.2' },
    {
      instructions:
        ...
        '**Year-end prep:** for "prepare my accounts for FY 2025/26" use `accountant_readiness_financial_year` (CT FY + 4 VAT quarters in one call). To resolve ambiguous period labels use `reporting_list_periods`; for "what is due soon" use `accountant_readiness_upcoming` with `deadline_horizon_days`. CT label = `2025/26`; VAT label = `Q{n}-{YYYY}`; date-range pack = regime `date_range` + `YYYY-MM_YYYY-MM` (e.g. `2025-01_2026-02`). **Packaging/ZIP/email is UI-only** (`/api/statements/download-for-accountant`) — MCP only diagnoses; `accountant_send_*` remain stubs. ' +
